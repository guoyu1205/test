...
<el-divider />
    ...
    <!-- <div>
     ...
     <el-text class="mx-1" style="margin-left:40px" type="success">负责教师：{{infoStore.info.standardName}}</el-text>
     <el-text class="mx-1" style="margin-left:40px" type="success">电话号码：{{infoStore.info.standardNumber}}</el-text>
     <div style="margin-top: 20px;">
     <el-text class="mx-1" style="margin-left:40px" type="success">实验名称：{{infoStore.pro.projectName}}</el-text>
     <el-text class="mx-1" style="margin-left:40px" type="success">实验室：{{infoStore.pro.detailNumber}}</el-text>
     </div>
    </div> -->
  ...
  >  
    <template #trigger>
      <el-button type="primary">上传预习报告</el-button>
    </template>
    </el-upload>
    <!-- 显示规程pdf -->
   ...
   ref="upload"
    ...
    action="/api/upload"
            ...
            :headers="{'Authorization':tokenStore.token}"
    accept = ".pdf,.PDF"
    :on-success="UploadSuccessTest"
  >  
    <template #trigger>
      <el-button type="primary">选择对比测试</el-button>
    </template>
    </el-upload>
    显示测试pdf 
    <div class="link-container">
...
<el-divider />
  ...
  <div>
   ...
   >
    <el-button  plain size="" type="primary">点击上传视频</el-button>
  </el-upload> -->
        <el-button style="margin-left: 110px;" type="danger" @click="handleDelete(scope.row.fileId)">
          删除
        </el-button>
    </div>
      </template>
     ...
     <div style="margin-top:20px;margin-left:440px;display: flex;align-items: center; gap: 10px;">
      <!-- <el-button plain @click="dialogFormAddVisible = true" type="primary">
    + 新增视频
  </el-button> -->
  <el-upload
     class="upload-demo"
    ...
    action="/api/upload"
     ...
     style="width: 100px;margin-left:40px"
   >
    <el-button  plain size="" type="primary">上传视频(新增)</el-button>
  </el-upload>
  <el-button type="primary" style="margin-left: 80px;" @click="reback">返回</el-button>
    </div>

<!-- 弹出添加表单
      <el-dialog v-model="dialogFormAddVisible" title="新增设备" width="500" center>
  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="isEmpty">
          提交
        </el-button>
      </div>
    </template>
...
const UploadSuccess = (result)=>{
    editUrl(result.data,1)
}
const UploadSuccessTest = (result)=>{
    editUrl(result.data,2)
}
const videoSuccess = (result)=>{
    addUrl(result.data,4)
    fileList();
}
// interface device {
//   deviceId: number
