...
const isEmpty = async () => {
  if (
    ...
    formAdd.deviceName = "";
    formAdd.manufacturer = "";
    (formAdd.deviceURL = ""), (formAdd.detail = "");
...
<template>
  ...
  <!-- <div>
    ...
    <el-text
      ...
      style="margin-left:100px"
      type="success"
    >项目名称：{{infoStore.pro.projectName}}</el-text>
    <el-text
      class="mx-1"
      style="margin-left:100px"
      type="success"
    >标准编号（含年号）及条款号：{{infoStore.pro.detailNumber}}</el-text>
  </div> -->
  ...
  <div>
    ...
    >
      ...
      <!-- <el-table-column prop label="操作" width="150px" fixed="right">
        <template #default="{row}">
          <el-button plain size="small" @click="handleEdit(row)" type="warning">申请设备</el-button>
        </template>
      </el-table-column> -->
    </el-table>
  </div>
