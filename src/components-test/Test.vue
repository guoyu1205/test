<template>
  ...
  <div class="table-container">
    ...
    >
      ...
      <el-table-column prop="planDate" label="计划时间" align="center" min-width="100" />
      <el-table-column prop="finishDate" label="完成时间" align="center" min-width="100" />
      <el-table-column prop="status" label="状态（计划、完成）" align="center" min-width="120">
...
// 表格数据
const tableData: CompareList[] = [
  {
    userName: '小强',
    category: '医用一次性防护服',  
    projectName: '撕破强力',
    standardName: '医用一次性防护服',
    detailNumber: 'GB 19082-2023',
    planDate: '2024-12-16',
    finishDate: '2024-12-16', status: '完成' 
  },
...
// 修改状态功能
const handleStatusChange = (row: CompareList) => {
  row.status = '完成'
  // 设置完成时间为当前日期
...
const exportToExcel = () => {
  ...
  try {
    ...
    // 添加工作表到工作簿
    XLSX.utils.book_append_sheet(wb, ws, '比对测试申请清单')

    // 导出文件
    const fileName = `比对测试申请清单_${new Date().toLocaleDateString()}.xlsx`
